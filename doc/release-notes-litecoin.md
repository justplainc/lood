Lood Core version *0.15.1* is now available from:

  <https://download.lood.org/lood-0.15.1/>
...
A number of changes to the way Lood Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 loods per kB (0.00001 LOOD). This is to prepare for dropping the minimum transaction fee to 0.1 loods per kB (0.0001 LOOD) in the next release.
...
- #11433 `e169349` Restore lood-util-test py2 compatibility (MarcoFalke)
- #11308 `2e1ac70` zapwallettxes: Wait up to 3s for mempool reload (MarcoFalke)
- #10798 `716066d` test lood-cli (jnewbery)
...
- #11125 `812c870` Add lood-cli -stdin and -stdinrpcpass functional tests (promag)
- #11077 `1d80d1e` fix timeout issues from TestNode (jnewbery)
- #11078 `f1ced0d` Make p2p-leaktests.py more robust (jnewbery)
- #11210 `f3f7891` Stop test_lood-qt touching ~/.lood (MeshCollider)
...
### Miscellaneous
- #11377 `75997c3` Disallow uncompressed pubkeys in lood-tx [multisig] output adds (TheBlueMatt)
