Lood Core version 0.12.0 is now available from:

  <https://lood.org/bin/lood-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/Lood-Qt (on Mac) or
loodd/lood-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Lood Core or other software:
...
pre-0.12 versions of Lood Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Lood Core version 0.11 or
earlier.
...
Lood Core 0.12 will have a strict maximum size on the mempool. The
...
Lood Core 0.12 also introduces new default policy limits on the length and
...
Lood Core 0.12 nodes. Lood Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/lood/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Lood Core 0.12 does not yet have support for
...
notes of v0.11.0](https://github.com/lood/lood/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
- #6214 `87406aa` [OSX] revert renaming of Lood-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new lood-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include lood-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix lood-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of lood-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [lood-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libloodconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "LOOD" (Ross Nicoll)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/lood/).
