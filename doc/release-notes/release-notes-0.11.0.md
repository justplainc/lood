Lood Core version 0.11.0 is now available from:

  <https://lood.org/bin/lood-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer LOOD/kB fee to be
...
For example, add the following to `lood.conf`:
...
related to the blockchain in the lood system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Lood Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Lood Core will 
...
For an example script see [lood-submittx](https://github.com/laanwj/lood-submittx).
...
- #5334 `cf87536` libloodconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new lood-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only LoodAmountField
- #5753 `0f44672` Add lood logo to about screen
...
- #5809 `46bfbe7` Add lood-cli man page
...
- #5849 `41259ca` contrib/init/loodd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Lood-Qt.app to Lood-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for lood-tx)
