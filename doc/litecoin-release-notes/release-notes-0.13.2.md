Lood Core version 0.13.2 is now available from:

  <https://download.lood.org/lood-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a lood
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Lood Core
randomly crashing on Windows XP. It is [not clear](https://github.com/lood/lood/issues/7681#issuecomment-217439891)
...
ECDSA signatures inside Lood transactions now use validation using
[libsecp256k1](https://github.com/lood-core/secp256k1) instead of OpenSSL.
...
(https://github.com/lood/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Lood Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Lood Core was to raise the default minimum
relay fee.

Lood Core 0.13.2 will have a strict maximum size on the mempool. The
...
Lood Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running loodd without having to do any manual configuration.
...
Lood Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Lood Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Lood Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Lood Core 0.13, and it is at this time undecided whether the
...
Lood Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Lood Core automatically creates a hidden service to listen on, without
manual configuration. Lood Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Lood Core is listening, and
...
Loodd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Lood
Core to use floating fees. By default, Lood Core will use floating
fees.
...
(default: `0.0002` LOOD/kB).

At all times, Lood Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LOOD.
Furthermore, Lood Core will never create transactions paying less than
...
`lood.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of Lood Core v0.11.0](https://github.com/lood/lood/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/lood/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/lood/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /loodrpc>
        ...
        # optional bypass loodd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< loodrpc:<password>
    </Location>
...
Restarting loodd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in lood.conf
...
For example:
    ...
    $ src/lood-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Lood Core code base has
...
- `lood-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `lood-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/lood/lood/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the Lood scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/lood/lood/pull/7524>
...
Lood transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/lood/lood/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/lood/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Lood Core.

[Pull request](https://github.com/lood/lood/pull/8035/files), [BIP 32](https://github.com/lood/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/lood/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/lood/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Lood-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/lood/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/lood/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/lood/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/lood/lood/pull/7762).
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Lood wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Lood block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of loods they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of loods being spent is exactly
...
- **Increased security for multisig:** Lood addresses (both P2PKH addresses
  ...
  believed to be provided by Lood's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Lood paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Lood's current ledger (called the UTXO set),
  ...
  note that Lood Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Lood users to individually opt-in to almost any change in the Lood
  Script language when those users receive new transactions.  Features
  currently being researched by Lood and Lood Core contributors that may
  ...
  Other Lood community members are studying several other improvements
...
segwit's start date of 1 January 2017 miners can update the Lood
client to Lood Core 0.13.2 to signal for segwit support. When a
...
[Segwit FAQ]: https://loodcore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://loodcore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/lood/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/lood/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/lood/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/lood/bips/blob/master/bip-0145.mediawiki
...
Since Lood Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/lood/bips/blob/master/bip-0147.mediawiki
...
- `lood-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `lood-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
