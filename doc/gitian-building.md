...
*Setup instructions for a Gitian build of Lood Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Lood
...
- [Building Lood Core](#building-lood-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-lood-host/README.md).
...
Clone the git repositories for lood and Gitian.
...
in the lood repository under 'Fetch and create inputs' to install sources which require
...
To build Lood Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the lood repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lood/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/lood-project/lood
...
For example:
```bash
URL=https://github.com/thrasher-/lood.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit lood=${COMMIT} --url lood=${URL} ../lood/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lood=${COMMIT} --url lood=${URL} ../lood/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lood=${COMMIT} --url lood=${URL} ../lood/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lood git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lood/contrib/gitian-descriptors/*|sort|uniq )
...
LOODPATH=/some/root/path/lood
SIGPATH=/some/root/path/lood-detached-sigs

./bin/gbuild --url lood=${LOODPATH},signature=${SIGPATH} ../lood/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lood-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lood-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lood-osx-build.assert
```
...
[lood-project/gitian.sigs.ltc](https://github.com/lood-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
