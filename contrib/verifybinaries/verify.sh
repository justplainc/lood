...
###   loodcore.org and lood.org and compares them.
...
WORKINGDIR="/tmp/lood_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://loodcore.org"
HOST2="https://lood.org"
BASEDIR="/bin/"
VERSIONPREFIX="lood-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'lood-',
...
if [ $? -ne 0 ]; then
   echo "lood.org failed to provide signature file, but loodcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "lood.org and loodcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Lood Core binary release signing key installed?"
   fi
