...
The RPM spec file provided here is for Lood-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://lood.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Lood git repository](https://github.com/lood/lood) but are not part of
...
Currently only `Source10` is used, for the example `lood.conf` file.
...
in the lood git repository. At present only an SVG version of the Lood
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the lood
...
needed when 0.12.1 is released, a proper fix is already in the Lood git
master branch.
...
idea to build Lood Core against a static version of that library compiled
...
The `lood.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `lood-server` package. At this point, most RPM based
...
The `lood-server` package should have SELinux support. How to properly do
...
Lood-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/lood/). He has been
packaging Lood for Fedora at least since 2012.
...
4. I renamed the `lood` package that contains the Qt GUI to `lood-core` as
that appears to be how the general population refers to it, in contrast to
`lood-xt` or `lood-classic`. I wanted to make sure the general population
