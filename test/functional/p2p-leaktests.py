...
Also test that nodes that send unsupported service bits to loodd are disconnected
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if loodd ban behavior changes
    def on_open(self, conn):
        super().on_open(conn)
...
class P2PLeakTest(LoodTestFramework):
