             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            ...
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CLoodAddress(strAddress).Get()].name;
        }
        else if (strType == "purpose")
        {
            std::string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CLoodAddress(strAddress).Get()].purpose;
        }
        else if (strType == "tx")
        {
            uint256 hash;
        ...
        else if (strType == "destdata")
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CLoodAddress(strAddress).Get(), strKey, strValue))
            {
