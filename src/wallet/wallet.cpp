...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CLoodAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/lood/lood/pull/10340, but that change has been
    // postponed until after https://github.com/lood/lood/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-lood-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CLoodAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CLoodAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CLoodAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CLoodAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CLoodAddress(address).ToString());
}
 ...
 * https://loodtalk.org/?topic=54527, or
 * https://github.com/lood/lood/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CLoodAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CLoodAddress(dest).ToString(), key);
}
