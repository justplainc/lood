...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total lood balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwallet && pwallet->GetCScript(scriptID, subscript)) {
            ...
            for (const CTxDestination& addr : addresses)
                a.push_back(CLoodAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given lood address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The lood address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The lood address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Lood address and we have full public key:
        CLoodAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are lood addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) lood address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The lood address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "lood-cli and the GUI. There is no server-side difference."
        );
